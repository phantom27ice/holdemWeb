...
## Sprint 8（已完成，Phase H）

1. 已完成：H1（加注金额交互，支持区间输入并在 store 侧做合法夹取）。
2. 已完成：H2（全下按钮与合法性联动）。
3. 已完成：H3（加注快捷档位与交互提示打磨）。
4. 已完成：H4（下注交互回归收口与文档封版）。

## Sprint 9（已完成，Phase I）

1. 已完成：I1（摊牌结果面板，含分池明细与牌型文案）。
2. 已完成：I2（结算面板居中放大、淡入淡出节奏、小屏遮挡优化）。
3. 已完成：I3（结算可解释性回归、回归门禁与文档封版）。
...
8. [x] Phase H 完成（H1/H2/H3/H4 全部完成）
9. [x] Phase I 完成（I1/I2/I3 全部完成）
...
目标：把 Hero 下注交互从“固定最小加注”升级为“可控金额 + 可解释约束”的完整下注面板。

范围：

1. 加注金额输入（滑杆/数字输入）与最小/最大区间约束。
2. 全下动作入口与合法动作联动。
3. 快捷金额档位与交互反馈（边界提示、超限自动夹取提示）。

执行规范：

1. 严格按 `PHASE_H_BETTING_UI_SPEC.md` 的 H1 -> H2 -> H3 -> H4 实施。
2. 每阶段完成后同步更新测试、规格与日志。

## 9. 新增阶段（Phase I）

目标：把牌局结算从“只看到筹码变化”升级为“可解释的摊牌结果”，明确谁以什么牌型赢得哪个池。
...
1. 严格按 `PHASE_I_SHOWDOWN_SPEC.md` 的 I1 -> I2 -> I3 实施。
2. 每阶段完成后同步更新测试、规格与日志。
